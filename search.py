...
def gap_squared(kfrac, c_, a12, p):
    bands = np.linalg.eigvalsh(hamiltonian(kfrac, c_, a12, p))
    gap_squared = math.pow(bands[2] - bands[1], 2.0)
    return gap_squared


def find_nodes(kfrac, c_, a12, p):
    ...
    result = minimize(fun=gap_squared, args=(c_, a12, p), x0=start, 
